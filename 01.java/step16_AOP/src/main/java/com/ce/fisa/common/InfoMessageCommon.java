...
//step01 : before -> after -> afrer return -> after throws
// 공통로직으로만 구성된 클래스
//@Component
//@Aspect
public class InfoMessageCommon {
	...
	//biz 메소드의 반환값 받고 처리
	@AfterReturning(pointcut = "execution(* com.ce.fisa.biz.Car.buyReturn(..))",
					returning = "bizReturnValue")
	public void afterReturn(String bizReturnValue) {
		System.out.println("공통 2 "+ bizReturnValue + " & 공통");
	}
	
	//예외 발생시에만 실행
	//biz 로직의 예외처리 전담 공통 메소드
	@AfterThrowing(pointcut = "execution(* com.ce.fisa.biz.Car.buyReturn(..))",
				   throwing = "e")
	public void throwsAfter(Exception e) {
		System.out.println("biz의 예외 발생시 공통 처리 로직 ********");
		e.printStackTrace();
	}
